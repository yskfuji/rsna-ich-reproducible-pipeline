"""Sweep simple postprocessing: CC min_size and top_k at a fixed probability threshold.
...
We:
- Run sliding-window inference once per case to get probability map.
- Binarize with `--threshold`.
- Label connected components once per case.
- Re-aggregate metrics for multiple (min_size, top_k) combinations without extra inference.

Outputs one evaluation directory per (min_size, top_k) under out_dir_base so it plugs into
reports/make_eval_report.py.

Example:
  PYTHONPATH=$PWD python -m src.evaluation.sweep_postprocess \
    --model-path runs/3d_unet/.../best.pt \
    --csv-path data/splits/...csv \
    --root data/processed/... \
    --split test \
    --out-dir-base results/postprocess_sweep_thr080 \
    --threshold 0.80 \
    --min-sizes 0,20,25,30,50,100,200 \
    --top-ks 0,1,3,5
"""
...
def _parse_patch_size(s: str) -> Tuple[int, int, int]:
    ps = [int(x) for x in str(s).split(",") if x.strip()]
    if len(ps) != 3:
...
def _component_stats(lbl: NDArray[np.int64], gt: NDArray[np.uint8]) -> tuple[NDArray[np.int64], NDArray[np.int64]]:
    flat_lbl = lbl.ravel()
    comp_sizes = np.bincount(flat_lbl).astype(np.int64, copy=False)
...
def _aggregate(
    ...
    n = int(comp_sizes.shape[0])
    if n <= 1:
        pred_vox = 0
        tp_vox = 0
        fp_vox = 0
        fn_vox = int(gt_vox)
        dice = float((eps) / (float(gt_vox) + eps))
    ...
    comp_ids = np.arange(n, dtype=np.int64)
    keep = (comp_ids > 0) & (comp_sizes >= int(min_size))

    kk = int(top_k)
    if kk > 0:
        cand_ids = np.where(keep)[0]
        if cand_ids.size > kk:
            cand_sizes = comp_sizes[cand_ids]
            top_ids = cand_ids[np.argsort(cand_sizes)[::-1][:kk]]
...
def main() -> None:
    ...
    p.add_argument("--top-ks", required=True, help="comma-separated ints; 0 disables (keep all)")
    ...
    p.add_argument("--tta", default="full", help="full|flip|none")

    args = p.parse_args()

    thr = float(args.threshold)
    min_sizes = sorted(set(_parse_int_list(args.min_sizes)))
    top_ks = sorted(set(_parse_int_list(args.top_ks)))

    ps = _parse_patch_size(args.patch_size)
    overlap = float(args.overlap)

    tta_mode = str(args.tta).strip().lower()
    if tta_mode not in {"full", "flip", "none", "off", "false", "0"}:
        raise ValueError(f"Unknown --tta: {args.tta!r}")

    temp = float(args.temperature)
    if not np.isfinite(temp) or temp <= 0:
        raise ValueError(f"temperature must be finite positive, got: {temp}")
    ...
    per_case: dict[tuple[int, int], list[dict]] = {(ms, tk): [] for ms in min_sizes for tk in top_ks}
    ...
    for sample in ds:
        ...
        for ms in min_sizes:
            for tk in top_ks:
                agg = _aggregate(comp_sizes, comp_tp, gt_vox=gt_vox, gt_pos=gt_pos, min_size=ms, top_k=tk)
                per_case[(ms, tk)].append(
                    {
    ...
    for (ms, tk), rows in per_case.items():
        out_dir = out_base / f"cc{int(ms)}_topk{int(tk)}"
    ...
    print(f"Wrote postprocess sweep under: {out_base}")
